 ...
 *  4. Set bonus stats (only when all 5 slots are filled)
 *  5. Echo skill from slot 1 — replaces the placeholder ECHO action in character.actions
 *  6. Modifier injection via resolveGear (slot-1 injectedModifiers/injectedSideEffects + set bonus injectedModifiers)
 */
export function resolveCharacter(character: Character): void {
  ...
  // ---- 4: Set bonus stats (all 5 slots must be filled) ----
  ...
  // ---- 5: Overwrite character stats ----
  character.stats = resolved

  // ---- 6: Inject echo skill from slot 1 ----
